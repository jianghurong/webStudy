...
2.**SessionStorage**在**浏览器窗口关闭删除**,大小限制和LocalStorage一样是**5M**  
...
ps：HTTP协议未规定GET和POST的长度限制,GET的长度限制是因为浏览器和Web服务器限制了URL长度
IE限制最短：为2083字节（bytes）
...
---
## 11.iframe有哪些缺点  
1.搜索引擎**无法解析**iframe框架，不利于SEO  
2.**阻塞window.onload事件**
3.**iframe和主页共享连接池**,浏览器对相同域的连接有限制,会影响并行加载
解决: 给iframe框架动态添加src属性,可以解决前两个问题:
```js
...
notice：通用选择器(\*),复合选择器(+,>,~,'')和否定伪类(:not)在权重（专用性）中无影响  
```css
/* 权重为3 */
h1 p::first-letter h1(1) + p(1) + ::first-letter(1)
/* 权重为2 */
li a[href="#"]
```
...
9. 页面开始**渲染DOM元素**，**JavaScript开始操作DOM例如绑定事件**等，页面显示完成
> 开启线程 => DNS => 发出请求 => 服务器 => 寻找请求处理 => 返回响应 => 下载资源 => dom 建立，请求文件 => 页面渲染
...
|XSS|攻击者向web页面插入恶意代码|1.限制字符串长度<br>2.对HTML元素(如"<",">"等)进行转义处理<br>3.对重要的cookie添加HttpOnly属性,限制JavaScript操作cookie<br>4.cookie加上secure字段限制只能使用https协议发送到服务器(增加攻击成本)
...
---
## 18.performance.timing API
[查看详情](https://javascript.ruanyifeng.com/bom/performance.html)